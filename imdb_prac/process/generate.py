...
from imdb_prac.process.text import rm_punct, strip_html,en_nlp
import pandas as pd
...
def main(args):
    ...
    if args.subparser_name=="data":
        ...
        #load from gcloud
        src_blob=bucket.get_blob(args.src)
        with src_blob.open("r") as f_r:
            df=pd.read_csv(f_r)
...
if __name__=="__main__":
    ...
    parser_data.add_argument("-n","--num",type=int,help="The data num you want to extract.")
    ...
    parser_token.add_argument("src",type=str,help="The path for training tokenizer's data on gcloud storage.")
    parser_token.add_argument("dst",type=str,help="THe path for save tokenizer on gcloud storage.")
    parser_token.add_argument('-mf',"--minFreq",type=int,default=1,help="The word leaset freq in dataset.")
    parser_token.add_argument("-ms","--maxSize",type=int,default=25000,help="Max vocabulary size for tokenizer.")
